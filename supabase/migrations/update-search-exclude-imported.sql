-- Migration: Update search_circuits_weighted to use batch_import_id instead of exclude_user_id
-- This fixes the "hide bulk imported" filter after changing attribution to use actual GitHub users

DROP FUNCTION IF EXISTS search_circuits_weighted(TEXT, TEXT, TEXT, TEXT, UUID, INT, INT);
DROP FUNCTION IF EXISTS search_circuits_weighted(TEXT, TEXT, TEXT, TEXT, UUID, BOOLEAN, INT, INT);
...
CREATE OR REPLACE FUNCTION search_circuits_weighted(
  ...
  exclude_user_id UUID DEFAULT NULL,  -- Deprecated, kept for backwards compatibility
  exclude_imported BOOLEAN DEFAULT FALSE,  -- New: filter by batch_import_id
...
BEGIN
  ...
  WHERE
    ...
    -- Exclude bulk-imported circuits (those with batch_import_id set)
