...
const R2_PUBLIC_URL = process.env.NEXT_PUBLIC_R2_PUBLIC_URL || 'https://pub-5cfb1ad5b22e451db2e5711b584b49c9.r2.dev';
 ...
 */
export function toR2ThumbnailUrl(url: string | null | undefined): string | null {
  if (!url) return null;
  ...
  try {
    ...
    const match = url.match(/\/thumbnails\/[^/]+\/([a-f0-9-]+)-v\d+-(light|dark)\.png/i);
 ...
 */
export function getR2ThumbnailUrls(circuitId: string): { light: string; dark: string } {
  return {
