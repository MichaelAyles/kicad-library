  ...
  try {
    ...
    for (const child of tree.children) {
      if (child.type === 'list' && child.children) {
        ...
        if (tag.type === 'atom' && (tag.value === 'lib_symbols' || tag.value === 'symbol')) {
    ...
    const libSymbolsMatch = fullFile.match(/(\(lib_symbols[\s\S]*?\n\)(?=\s*\n\s*\())/);
    const symbolsMatch = fullFile.match(/(\(symbol[\s\S]*?\n\s*\)(?=\s*\n\s*\())/g);
 ...
 */
export function addAttribution(fullFile: string, options: {
  author: string;
  url: string;
  license: string;
  title?: string;
...
export function validateSExpression(sexpr: string): ValidationResult {
  ...
  if (!sexpr || sexpr.trim().length === 0) {
    ...
    return { valid: false, errors, warnings, isSnippet: false, originalFormat: 'full' };
  }
  ...
  if (isSnippet) {
    warnings.push('Clipboard snippet detected - will be stored as-is, wrapped for preview');
  ...
  try {
    ...
    if (!version || version < 20211014) {
      errors.push('KiCad 5 or earlier format detected. Please use KiCad 6 or later.');
      return { valid: false, errors, warnings, isSnippet, originalFormat };
    }
    ...
    errors.push(`Failed to parse schematic: ${error instanceof Error ? error.message : 'Unknown error'}`);
    return { valid: false, errors, warnings, isSnippet, originalFormat };
  }
}
...
export function suggestTags(metadata: ParsedMetadata): string[] {
  ...
  // Voltage detection
  const allValues = metadata.components.map(c => c.value).join(' ').toLowerCase();
