...
const RegenerateThumbnailSchema = z.object({
  ...
  lightThumbBase64: z.string().startsWith('data:image/', 'Light thumbnail must be a data URL'),
  darkThumbBase64: z.string().startsWith('data:image/', 'Dark thumbnail must be a data URL'),
});
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { circuitId, userId, lightThumbBase64, darkThumbBase64 } = validationResult.data;
    ...
    console.log(`[Thumbnail Regen] Circuit ${circuitId}: DB version=${dbVersion}, Storage version=${storageVersion}`);
    ...
    while (newVersion === null && retries < MAX_RETRIES) {
      const currentVersion = Math.max(circuit.thumbnail_version || 0, storageVersion);
    ...
    if (error.message?.includes('[STORAGE]')) {
      ...
      suggestion = 'The operation took too long. Try reducing the image size or try again later';
