...
function BrowsePageContent() {
  ...
  const [sortBy, setSortBy] = useState<
    "copies" | "recent" | "favorites" | "relevance"
  >("copies");
    ...
    const loadCircuits = async () => {
      ...
      try {
        if (activeQuery || activeCategory) {
          ...
          const { circuits: data } = await getCircuits(
            CIRCUITS_PER_PAGE,
            0,
            sortMap[sortBy],
            hideImported,
  ...
  const handleSortChange = (
    newSort: "copies" | "recent" | "favorites" | "relevance",
  ) => {
    setSortBy(newSort);
  };
            ...
            <div className="flex items-center gap-4 text-sm">
              ...
              {isSearchMode && (
                ...
                >
                  Relevance
                  {sortBy === "relevance" && (
                    <span className="absolute -bottom-1 left-0 w-full h-0.5 green-gradient" />
                  )}
                </button>
              )}
              <button
                  ...
                  >
                    ...
                    <div className="p-4">
                      ...
                      <div className="flex items-center justify-between text-sm text-muted-foreground">
                        <span>
                          by @
                          {("github_owner" in circuit && circuit.github_owner
              ...
              {isLoadingMore && (
                <div className="flex flex-col items-center gap-2">
                  ...
                  <p className="text-muted-foreground text-sm">
                    Loading more circuits...
                  </p>
                </div>
              )}
              {!hasMore &&
                !isLoadingMore &&
                circuits.length >= CIRCUITS_PER_PAGE && (
                  <p className="text-muted-foreground text-sm">
                    You&apos;ve reached the end! Showing all {circuits.length}{" "}
                    circuits.
                  </p>
                )}
            </div>
